...
description: "Using Google Earth Engine and Python to analyze vegetation changes due to fire via spectral indices and a time series analysis"
...
One of the most powerful uses of satellite remote sensing is detecting how land cover changes over long periods time. While one single image can tell you what an area looks like today, a time series writes the story of how it got there. In this lab, I used **Google Earth Engine (GEE)**, Landsat 8 imagery, and spectral indices to track vegetation change before and after the **2018 Woolsey Fire** in Southern California.

In November 2018, the Woolsey Fire burned roughly 97,000 acres across the Los Angeles and Ventura Counties, scorching the Santa Monica Mountains found between Thousand Oaks and Malibu. With a remote sensing background, I know a fire that large will leave a clear and highly-measurable signal in satellite data. The technique applied here is a **time series analysis**: pulling imagery across many dates, calculating a vegetation index for each image, and plotting how that index changes over time... all to track the fire's impact and the recovery of the landscape.

---

### What is Google Earth Engine?

Google Earth Engine is a free, cloud-based platform created specifically for analyzing satellite imagery. Normally, working with this kind of data involves downloading many large files to a hard drive and hoping your computer can handle everything. But GEE fixes this: the user writes code that runs on Google's servers, right where all of the imagery already lives. As a result, you can access and analyze decades of global satellite data in mere seconds, from any computer.
...
:::: {.columns}
::: {.column width="60%"}
Landsat is a joint NASA/USGS satellite program that has been consistently imaging Earth's surface since 1972. This makes it the longest continuous space-based record of Earth's land surface ever. Each satellite captures **multi-spectral imagery**, meaning it records light in multiple wavelength bands... beyond just visible colors. It captures near-infrared (NIR) and shortwave infrared (SWIR) bands, and these are what makes advanced vegetation and water analysis possible.
...
Rather than analyzing raw band values, remote sensing scientists calculate ratios of specific band combinations called spectral indices to highlight particular environmental features. The index used in this lab was:

**NDVI (Normalized Difference Vegetation Index)**

Calculated as the band ratio `(NIR - Red) / (NIR + Red)`, applying NDVI to image pixels returns values range from -1 to 1. Healthy green vegetation reflects strongly in near-infrared and absorbs red light, producing high positive values. Bare soil, water, and developed areas produce values near zero or negative.

---

### Code Walkthrough

Here is the workflow from the lab. Each step is annotated to explain what it does.
...
Rather than spending an hour to delineate a study area by hand, I loaded a publicly available Woolsey Fire boundary as a **GeoJSON** file from [CAL FIRE](https://gis.data.ca.gov/datasets/CALFIRE-Forestry::california-fire-perimeters-all/explore?location=38.012055%2C-118.825081%2C7){target="_blank"}. A GeoJSON is a standard file format that geographers use to store vectors (points, lines, polygons) as plain text that both humans and code can read.
...
To give that drop some more context, I added a second time series from a nearby unburned area in the Santa Monica Mountains as a baseline to compare. If the fire caused a real change in vegetation, beyond just normal seasonal variation, the two lines should noticeably diverge after the November 2018 date.
...
Before the Woolsey fire, both areas track each other pretty closely through the year. After the fire, the burned area drops hard while the unburned baseline maintains a generally flat line. There seems to be significant vegetation impacts from the fire. 

---

##### Step 5: LOWESS smoothing

Now, even with clean data, individual satellite images can still be thrown off by leftover haze, thin clouds, or other atmospheric noise. These show up as random spikes or dips in the scatter plot. To smooth them out, and without losing the raw data, I applied a technique called LOWESS (Locally Weighted Scatterplot Smoothing). It is similar to drawing a best-fit line through the data, but instead of just one straight line across everything, the algorithm fits a bunch of smaller, local curves that follow the shape of the data more naturally. The `frac` parameter controls how tight those local fits are. Here, I used a value of 0.15, meaning each smoothed point considers 15% of the surrounding data when fitting its curve.
...
As you can see, the smoothed curves make the story easy to read. The NDVI for the Fire Scare abruptly crashes in late 2018, starts to recover through the 2019 spring, and gradually returns to baseline patterns from there. The blue unburned baseline shows expected seasonal changes throughout the whole period. This visual gave me confidence in saying that the vegetation loss inside the fire perimeter was real and caused by the fire itself, and not a normal part of the season.
